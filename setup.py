        ...
        if not library_found:
            if sys.platform == "linux":
                ...
                library_found = library_found or os.path.exists(
                    f"/usr/local/include/{library_header}"
                )
...
def get_extensions():
    ...
    force_cuda = True  # Modified by @amirdonte15
    ...
    vision_include = "../Video_Codec_SDK_11.1.5/Interface"  # modified by @amirdonte15
    vision_library = os.environ.get("TORCHVISION_LIBRARY", None)
    vision_library = (
        "../Video_Codec_SDK_11.1.5/Lib/linux/stubs/x86_64"  # modified by @amirdonte15
    )
    vision_include = (
    ...
    video_codec_found = (
        ...
        and any(
            [
                os.path.exists(os.path.join(folder, "libnvcuvid.so"))
